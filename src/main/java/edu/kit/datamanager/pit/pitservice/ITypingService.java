...
public interface ITypingService extends IIdentifierSystem{
  ...
  /**
   * Resolves the given PID without previous knowledge about the kind of entity
   * it identifies (e.g. a common PID record, a property or type definition
   * etc.).
   *
   * @param pid
   * @return The returned object can be either a PID record, a property
   * definition or a type definition. It can also be null, indicating the PID is
  ...
  /**
   * Queries all properties of a type available from the given PID. If optional
   * properties are present, they will be returned as well. If there are
   * mandatory properties missing (i.e. the record of the given PID does not
   * fully conform to the type), the method will NOT fail but simply return only
   * those properties that are present.
   *
   * @param pid
   ...
   * values. Contains all property values present in the record of the given PID
   * that are also specified by the type (mandatory or optional). If the pid is
   ...
   * @return a PID information record with property information and conformance
   * check results.
   * @throws IOException
   * @throws InconsistentRecordsException
   */
