...
# VALIDATE PID PROFILE
echo "check if PID is matching the given profile"

response=$(curl --head \
  --url "http://localhost:8090/api/v1/pit/profile/21.T11148/301c6f04763a16f0f72a?identifier=$pid" \
  --include)
...
if [ "$wasSuccessful" -eq 0 ]
then
    echo "FAILED: API claims PID does not match the profile. Response:"
    echo "$response"
    exit 1  # failure
fi
